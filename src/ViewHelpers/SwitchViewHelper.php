...
class SwitchViewHelper extends AbstractViewHelper {
	...
	public function render() {
		...
		$variableContainer->addOrUpdate('TYPO3\Fluid\ViewHelpers\SwitchViewHelper', 'switchExpression', $expression);
		$variableContainer->addOrUpdate('TYPO3\Fluid\ViewHelpers\SwitchViewHelper', 'break', FALSE);

		$content = $this->retrieveContentFromChildNodes($this->childNodes);

		if ($variableContainer->exists('TYPO3\Fluid\ViewHelpers\SwitchViewHelper', 'switchExpression')) {
			$variableContainer->remove('TYPO3\Fluid\ViewHelpers\SwitchViewHelper', 'switchExpression');
		}
		if ($variableContainer->exists('TYPO3\Fluid\ViewHelpers\SwitchViewHelper', 'break')) {
			$variableContainer->remove('TYPO3\Fluid\ViewHelpers\SwitchViewHelper', 'break');
		}
	...
	protected function retrieveContentFromChildNodes(array $childNodes) {
		...
		foreach ($this->childNodes as $childNode) {
			...
			if ($this->viewHelperVariableContainer->get('TYPO3\Fluid\ViewHelpers\SwitchViewHelper', 'break') === TRUE) {
				$defaultCaseViewHelperNode = NULL;
				break;
			}
		}
	...
	protected function isDefaultCaseNode(NodeInterface $node) {
		return ($node instanceof ViewHelperNode && $node->getViewHelperClassName() === 'TYPO3\Fluid\ViewHelpers\DefaultCaseViewHelper');
	}
	...
	protected function isCaseNode(NodeInterface $node) {
		return ($node instanceof ViewHelperNode && $node->getViewHelperClassName() === 'TYPO3\Fluid\ViewHelpers\CaseViewHelper');
	}
