...
class SelectViewHelper extends F3::Beer3::Core::TagBasedViewHelper {
	...
	protected function callSpecifiedProperty($object, $name) {
		if (!is_object($object)) throw new F3::Beer3::RuntimeException('SelectViewHelper expects a list of objects if you specify the "optionKey" argument.', 1227711174);
		...
		if (method_exists($object, $getterMethodName)) {
			...
			throw new F3::Beer3::RuntimeException('Tried to call "' . $getterMethodName . '" on an object in the select view helper but method does not exist.', 1227711306);
		}
	}
}
