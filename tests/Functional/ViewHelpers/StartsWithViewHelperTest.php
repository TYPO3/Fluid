...
final class StartsWithViewHelperTest extends AbstractFunctionalTestCase
{
    public static function renderDataProvider(): \Generator
    {
        yield 'empty search and subject, search true' => [
            '<f:startsWith search="{search}" subject="{subject}" then="thenArgument" />',
            ['search' => '', 'subject' => ''],
            'thenArgument',
        ];
        ...
        yield 'empty subject string, no then, search false' => [
            '<f:startsWith search="{search}" subject="{subject}" else="elseArgument" />',
            ['search' => 'search', 'subject' => ''],
            'elseArgument',
        ];
        ...
        yield 'search and subject given, search false' => [
            '<f:startsWith search="{search}" subject="{subject}" then="thenArgument" else="elseArgument" />',
            ['search' => 'search', 'subject' => 'subject'],
            'elseArgument',
        ];
        yield 'leading whitespace, empty result body, search false' => [
            ' <f:startsWith search="{search}" subject="{subject}"><f:variable name="foo" value="bar" /></f:startsWith>',
            ['search' => 'search', 'subject' => 'subject'],
            ' ',
        ];
        yield 'result body, search true' => [
            '<f:startsWith search="{search}" subject="{subject}">Foo</f:startsWith>',
            ['search' => 'search', 'subject' => 'search'],
            'Foo',
        ];
        ...
        yield 'inline syntax, with pipe, search true' => [
            '{my_variable -> f:startsWith(search:\'{search}\')}',
            ['search' => 'test', 'my_variable' => 'testing'],
            'testing',
        ];
        ...
        yield 'inline syntax, with pipe, with subject, search true' => [
            '{my_variable -> f:startsWith(search:\'{search}\', subject: \'{subject}\')}',
