...
:doc:`the documentation about using ViewHelpers </Usage/ViewHelpers>`. And the inline mode is designed to access variables, use
...
in inline mode (e.g. `{myArray -> f:count()}`). Expressions are written as *variable access with additional syntax*:
...
read more about in :ref:`the implementation chapter, section about ViewHelperResolver <implementation-view-helper-resolver>`.
You can `view the built-in expression types <https://github.com/TYPO3/Fluid/tree/main/src/Core/Parser/SyntaxTree/Expression>`__ at any time, but if you use Fluid
...
don't always have the option of *assigning a properly typed variable like FALSE that you can use when a ViewHelper wants a boolean value*, which would be the strict way of passing a boolean. To accommodate this, Fluid will convert compatible types into
