...
`<f:render>` tag. Both Templates and Partials may both contain and render sections, but Layouts may only **render** sections.
...
Note that paths are *always defined as arrays*. In the default `TemplatePaths` implementation, Fluid supports lookups in multiple
...
a few template files. By adding your own template files path *last in the paths arrays* Fluid will check those paths *first*.
...
* When a Layout is used, *the Template itself is not rendered directly*. Instead, the Template defines any number of `<f:section>`
...
design with just smaller pieces being interchangeable (which most web applications do) your Layout can contain the container HTML
